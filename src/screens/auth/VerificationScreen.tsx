...
const Verification = ({navigation, route}: any) => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setLimit(120);
      setCurrentCode(res.data.metadata.code);
      setIsLoading(false);

      console.log(res.data.metadata.code);
    } catch (error) {
      setIsLoading(false);
  ...
  const handleVerification = async () => {
    //if (limit > 0) {
      ...
      if (parseInt(newCode) !== parseInt(currentCode)) {
          setErrorMessage('Invalid code!!!');
          console.log("sai r ",newCode, currentCode);
          
        } else { 
          console.log("dung r ",newCode, currentCode);
          ...
          icon={
            <View
              style={[
                ...
                {
                  backgroundColor:
                    newCode.length !== 4 ? appColors.gray3 : appColors.primary,
                },
              ]}>
